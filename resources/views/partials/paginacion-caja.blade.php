<div id="paginacion-movimientos-caja">
    ...
    <div>
        Mostrando
        {{ $paginador->firstItem() ?? 0 }}–{{ $paginador->lastItem() ?? 0 }}
        de {{ $paginador->total() }} movimientos
    </div>
    ...
    @if ($paginador->hasPages())
        ...
        <nav>
            ...
            @for (
                $pagina = max(1, $paginaActual - 2);
                $pagina <= min($ultimaPagina, $paginaActual + 2);
                $pagina++
            )
            ...
            @if ($paginaActual < $ultimaPagina - 2)
                <span>...</span>
                <a href="{{ $paginador->url($ultimaPagina) }}">
                    {{ $ultimaPagina }}
                </a>
            @endif
