...
@section('content')
    ...
    <style>
        ...
        .selector-proveedor-linea .campo-producto {
            flex: 1;
        }
        ...
        .mensaje-proveedor-exito {
            display: none;
            margin-bottom: 18px;
            padding: 12px 16px;
        }

        .errores-proveedor-modal.visible {
            display: block;
            background: #f8d7da;
    ...
    <form class="formulario-producto" method="POST" action="/productos">
        @csrf
        ...
        <fieldset class="grupo-producto">
            ...
            <div class="campo-producto">
                ...
                <input type="text" id="nombre" name="nombre" value="{{ old('nombre') }}" placeholder="Ej.: Coca Cola 1.5L" required>
            </div>
        ...
        <fieldset class="grupo-producto">
            ...
            <div class="campo-producto campo-precio-producto">
                ...
                <input type="number" id="precio_compra" name="precio_compra" value="{{ old('precio_compra') }}" min="0" step="0.01" required>
            </div>

            <div class="campo-producto campo-precio-producto">
                <label for="precio_venta">Precio de venta (opcional)</label>
                <input type="number" id="precio_venta" name="precio_venta" value="{{ old('precio_venta') }}" min="0" step="0.01">
            </div>
        </fieldset>
        ...
        <fieldset class="grupo-producto">
            ...
            <div class="campo-producto">
                ...
                <input type="number" id="stock_minimo" name="stock_minimo" value="{{ old('stock_minimo', 1) }}" min="0">
            </div>

            <div class="campo-producto">
                <label for="unidad">Unidad de medida (opcional)</label>
                <select id="unidad" name="unidad">
                    <option value="Unidad" {{ old('unidad', 'Unidad') === 'Unidad' ? 'selected' : '' }}>Unidad</option>
                ...
                <div class="campo-producto">
                    ...
                    <select id="proveedor" name="proveedor">
                        ...
                        @foreach ($proveedores as $proveedor)
                            <option value="{{ $proveedor->empresa }}" {{ old('proveedor') === $proveedor->empresa ? 'selected' : '' }}>
                                {{ $proveedor->empresa }}
                            </option>
                        @endforeach
                    </select>
        ...
        <fieldset class="grupo-producto">
            ...
            <div class="campo-producto">
                ...
                <select id="tiene_vencimiento" name="tiene_vencimiento">
                    <option value="0" {{ old('tiene_vencimiento', '0') === '0' ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('tiene_vencimiento') === '1' ? 'selected' : '' }}>Sí</option>
                </select>
            </div>
            ...
            <div id="fecha-vencimiento-contenedor" class="campo-producto oculto">
                ...
                <input type="date" id="fecha_vencimiento" name="fecha_vencimiento" value="{{ old('fecha_vencimiento') }}" disabled>
            </div>
        </fieldset>

        <button type="submit" class="boton-producto">Guardar producto</button>
    ...
    <div id="modal-nuevo-proveedor" class="modal-proveedor oculto" aria-hidden="true">
        <section class="modal-proveedor-contenido" role="dialog" aria-modal="true" aria-labelledby="nuevo-proveedor-titulo">
            ...
            <form id="form-nuevo-proveedor">
                @csrf
                <div class="campo-proveedor-modal">
                    <label for="proveedor-empresa">Empresa</label>
                    <input type="text" id="proveedor-empresa" name="empresa" required>
                </div>
                <div class="campo-proveedor-modal">
                    <label for="proveedor-contacto">Contacto (opcional)</label>
                    <input type="text" id="proveedor-contacto" name="contacto">
                </div>
                <div class="campo-proveedor-modal">
                    <label for="proveedor-telefono">Teléfono</label>
                    <input type="text" id="proveedor-telefono" name="telefono" required>
                </div>
                <div class="campo-proveedor-modal">
                    <label for="proveedor-email">Email (opcional)</label>
                    <input type="email" id="proveedor-email" name="email">
                </div>
                <div class="campo-proveedor-modal">
                    <label for="proveedor-direccion">Dirección (opcional)</label>
                    <input type="text" id="proveedor-direccion" name="direccion">
                </div>
                <div class="campo-proveedor-modal">
                    <label for="proveedor-cuit">CUIT (opcional)</label>
                    <input type="text" id="proveedor-cuit" name="cuit">
                </div>
    ...
    <script>
        ...
        function actualizarFechaVencimiento() {
            const tieneVencimiento = selectorVencimiento.value === '1';

            fechaVencimientoContenedor.classList.toggle('oculto', !tieneVencimiento);
            fechaVencimiento.disabled = !tieneVencimiento;
        }

        selectorVencimiento.addEventListener('change', actualizarFechaVencimiento);
        ...
        formularioNuevoProveedor.addEventListener('submit', function (event) {
            ...
            fetch('/proveedores', {
                ...
                aviso.className = 'mensaje-proveedor-exito visible';
                aviso.textContent = `✓ ${datos.message}`;
                document.querySelector('.formulario-producto').prepend(aviso);
                setTimeout(function () {
                    aviso.remove();
                }, 4000);
